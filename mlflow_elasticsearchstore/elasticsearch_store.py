...
class ElasticsearchStore(AbstractStore):
    ...
    def __init__(self, store_uri: str = None, artifact_uri: str = None) -> None:
        ...
        self.es_client = Elasticsearch(
            hosts=["http://" + urllib.parse.urlparse(store_uri).netloc]
        )
        self.es_client.info()  # Test connection
        # connections.create_connection(hosts=[urllib3.parse.urlparse(store_uri).netloc])
    ...
    def _hit_to_mlflow_experiment(self, hit: Any) -> Experiment:
        return Experiment(
            experiment_id=hit.meta.id,
            name=hit.name,
            artifact_location=hit.artifact_location,
            lifecycle_stage=hit.lifecycle_stage,
        )

    def _hit_to_mlflow_run(
        self, hit: Any, columns_to_whitelist_key_dict: dict = None
    ) -> Run:
        return Run(
            run_info=self._hit_to_mlflow_run_info(hit),
    ...
    def _hit_to_mlflow_run_info(self, hit: Any) -> RunInfo:
        return RunInfo(
            ...
            lifecycle_stage=hit.lifecycle_stage
            if hasattr(hit, "lifecycle_stage")
            else None,
            artifact_uri=hit.artifact_uri if hasattr(hit, "artifact_uri") else None,
        )

    def _hit_to_mlflow_run_data(
        self, hit: Any, columns_to_whitelist_key_dict: dict
    ) -> RunData:
        metrics = [
            self._hit_to_mlflow_metric(m)
            for m in (hit.latest_metrics if hasattr(hit, "latest_metrics") else [])
            if (
                columns_to_whitelist_key_dict is None
                or m.key in columns_to_whitelist_key_dict["metrics"]
            )
        ]
        params = [
            self._hit_to_mlflow_param(p)
            for p in (hit.params if hasattr(hit, "params") else [])
            if (
                columns_to_whitelist_key_dict is None
                or p.key in columns_to_whitelist_key_dict["params"]
            )
        ]
        tags = [
            self._hit_to_mlflow_tag(t)
            for t in (hit.tags if hasattr(hit, "tags") else [])
            if (
                columns_to_whitelist_key_dict is None
                or t.key in columns_to_whitelist_key_dict["tags"]
            )
        ]
    ...
    def _hit_to_mlflow_metric(self, hit: Any) -> Metric:
        return Metric(
            key=hit.key,
            value=float("nan")
            if (hasattr(hit, "is_nan") and hit.is_nan)
            else hit.value,
    ...
    def list_experiments(
        ...
        response = (
            Search(index="mlflow-experiments")
    ...
    def _get_artifact_location(self, experiment_id: str) -> str:
        return append_to_uri_path(self.artifact_root_uri, experiment_id)
    ...
    def create_experiment(self, name: str, artifact_location: str = None) -> str:
        ...
        if name in existing_names:
            raise MlflowException(
                "This experiment name already exists", INVALID_PARAMETER_VALUE
            )

        experiment = ElasticExperiment(
            name=name,
    ...
    def _check_experiment_is_active(self, experiment: ElasticExperiment) -> None:
        if experiment.lifecycle_stage != LifecycleStage.ACTIVE:
            raise MlflowException(
                f"The experiment {experiment.meta.id} must be in the 'active' state. Current state is {experiment.lifecycle_stage}.",
                INVALID_PARAMETER_VALUE,
            )
    ...
    def create_run(
        ...
        run_tags = [
            ElasticTag(key=key, value=value) for key, value in tags_dict.items()
        ]
        run = ElasticRun(
            meta={"id": run_id},
            run_id=run_id,
    ...
    def _check_run_is_active(self, run: ElasticRun) -> None:
        if run.lifecycle_stage != LifecycleStage.ACTIVE:
            raise MlflowException(
                f"The run {run.meta.id} must be in the 'active' state. Current state is {run.lifecycle_stage}.",
                INVALID_PARAMETER_VALUE,
            )
    ...
    def _check_run_is_deleted(self, run: ElasticRun) -> None:
        if run.lifecycle_stage != LifecycleStage.DELETED:
            raise MlflowException(
                f"The run {run.meta.id} must be in the 'deleted' state. Current state is {run.lifecycle_stage}.",
                INVALID_PARAMETER_VALUE,
            )

    def update_run_info(
        self, run_id: str, run_status: RunStatus, end_time: int
    ) -> RunInfo:
    ...
    def _get_run(self, run_id: str) -> ElasticRun:
        return ElasticRun.get(id=run_id)
    ...
    @staticmethod
    def _update_latest_metric_if_necessary(
        new_metric: ElasticMetric, run: ElasticRun
    ) -> None:
        def _compare_metrics(
            metric_a: ElasticLatestMetric, metric_b: ElasticLatestMetric
        ) -> bool:
        ...
        new_latest_metric = ElasticLatestMetric(
            key=new_metric.key,
    ...
    def _log_param(self, run: ElasticRun, param: Param) -> None:
        ...
        new_param = ElasticParam(key=param.key, value=param.value)
    ...
    def get_metric_history(self, run_id: str, metric_key: str) -> List[Metric]:
        s = (
            Search(index="mlflow-metrics")
            .filter("term", run_id=run_id)
    ...
    def _list_columns(
        ...
        s = (
            Search(index="mlflow-runs")
            .filter("match", experiment_id=experiment_id)
            .filter("terms", lifecycle_stage=stages)
        )
        s.aggs.bucket(column_type, "nested", path=column_type).bucket(
            f"{column_type}_keys",
            "composite",
            size=size,
    ...
    def list_all_columns(self, experiment_id: str, run_view_type: str) -> "Columns":
        columns: Dict[str, List[str]] = {"latest_metrics": [], "params": [], "tags": []}
        stages = LifecycleStage.view_type_to_stages(run_view_type)
        for column_type in ["latest_metrics", "params", "tags"]:
            self._list_columns(experiment_id, stages, column_type, columns[column_type])
        return Columns(
            metrics=columns["latest_metrics"],
            params=columns["params"],
            tags=columns["tags"],
        )

    def _build_columns_to_whitelist_key_dict(
        self, columns_to_whitelist: List[str]
    ) -> dict:
        if columns_to_whitelist is None:
            return None
        columns_to_whitelist_key_dict: dict = {
            "metrics": set(),
            "params": set(),
            "tags": set(),
        }
    ...
    def _build_elasticsearch_query(self, parsed_filters: List[dict]) -> List[dict]:
        ...
        for search_filter in parsed_filters:
            ...
            if key_type == "parameter":
                ...
                query_val = Q(
                    self.filter_key[comparator][0], params__value=filter_ops[comparator]
                )
            elif key_type == "tag":
                query_type = Q("term", tags__key=key_name)
                query_val = Q(
                    self.filter_key[comparator][0], tags__value=filter_ops[comparator]
                )
            elif key_type == "metric":
                query_type = Q("term", latest_metrics__key=key_name)
                query_val = Q(
                    self.filter_key[comparator][0],
    ...
    def _get_orderby_clauses(self, order_by_list: List[str]) -> List[dict]:
                    ...
                    sort_clauses.append(
                        {
                            f"{key_type}.value": {
                                ...
                                "nested": {
                                    "path": key_type,
                                    "filter": {"term": {f"{key_type}.key": key}},
                                },
                            }
                        }
                    )
        ...
        sort_clauses.extend(
            ({"start_time": {"order": "desc"}}, {"run_id": {"order": "asc"}})
        )
        return sort_clauses
    ...
    def _search_runs(
        ...
        page_token: str = None,
        columns_to_whitelist: List[str] = None,
    ) -> Tuple[List[Run], str]:
        if max_results > 10000:
            raise MlflowException(
                f"Invalid value for request parameter max_results. It must be at most 10000, but got value {max_results}",
                INVALID_PARAMETER_VALUE,
            )
        stages = LifecycleStage.view_type_to_stages(run_view_type)
    ...
    def update_artifacts_location(
        self, run_id: str, new_artifacts_location: str
    ) -> None:
        run = self._get_run(run_id=run_id)
